...
1. Tell me about a time that you have to make a decision between standard and delivery?
   ...
   Situation:    
   There was once a time that we had a production issue that a few products' effective dates and termination dates were calculated wrongly by the system. It caused one of our largets clients to have done some wrong trades and which needed to be revoked. The client was very unhappy and asked the company to solve the issue ASAP. As a developer who was very familiar with the system, I was asked to look into the issue and find a fix. I quickly found that these dates were calculated by the system roughly every three months and the core logic was flawed and hence the system could generate wrong dates from time to time. I also tried to understand how the dates were calculated but realized the logic was very complex. Ideally these logic of calcuating dates should be fixed so the issue would not appear again. However, that was not easy and undoubtedly would take much more time, and involed more risk as well. 
   
   Task:  
   So I was thinking what shall I do? To spend more time to come up with a solution that fixes the rootcause or have a short-time workaround to fix the issue temporariliy first? What happened in the past was often times when a issue was fixed by a workaound, it was forgotten and the underlying issue was not fixed, and at some point, the issue might come again. So back then, the company had a standard of avoding quick dirty workaround, but engineeres should aim to fix issue properly.  
   ...
   I also proposed that we needed to create a task to fix the issue more properly by fixing the flawed the logic of generating dates, and prioritized the task to be worked in the near future so we didn't forget about it.
   
   Result:  
   The management agreed with my suggestion. So I went on to fix the issue by provoding the workaround in just a day. The client was very pleased with the quick turnover. Later I also worked on fixing on the flawed logic of date generation so the dates would never be wrong again, not only for that product, but also for many other products which shared same logic for generating dates. What I learned sometime it was okay to lower the standard a little bit for quick delivery if doing so provide business value but be sure to make change to improve the standard later so to avoid accmulating technical debt.
