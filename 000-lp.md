...
1. Solved production quickly
   ...
   Situation:  
   We once had a urgent production issue in a morning that our clients couldn't trade in some markets. I was asked to lead a small team to look into this and find a fix urgently. After analyzing the code change that went into production recently, I quickly found the rootcause. The issue was caused by mis-configuration. The fix was simple but quite a few applications need to be restarted to let the change take effect. When applications restarted, potentially there could be more issues.

   Task:  
   So the challenge was shall we release the change mid of day or wait until end of day when market was closed?

   Action:  
   To help make a better decision, I wanted to find out what was the effect to restart different applications. I identified the applications that we needed to start, and based on the dependency, I defined orders of how application should be started. For example, when an application had both primary and secondary instance, we restarted the secondary instance first. Then I tested in a testing environment with the team, and found that the whole process was pretty quick and there was no issue at all. Although there was still risk that things didn't work well in production, but the likelihood was not very high. So I suggested to take this calculated risk to apply the fix in production immediately instead of waiting until end of day.

   Result:  
   The senior management then approved and the fix went to production. Turned out it was very successful as the urgent production issue was fixed and no new issue was introduced in the process. Our clients were very pleased with the quick turnaround. It also helped us to avoid any revenue loss or reputation damage. 
...
## Ownership
   something significant outside your area of responsibility and why was it important and what was the outcome okay
