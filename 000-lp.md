...
1. Tell me about a time that you have to make a decision between standard and delivery?
   ...
   I also proposed that we needed to create a task to fix the issue more properly by fixing the flawed the logic of generating dates, and prioritized the task to be worked in the near future so we didn't forget about it like before. 
   
   Result:  
   The management agreed with my suggestion. So I went on to fix the issue by provoding the workaround and released the change in just two days. The client was very pleased with the quick turnraoud. A few days later I was also able to fix the flawed logic of dates generation so the dates would never be wrong again, not only for that product, but also for many other products which shared same logic for generating dates. What I learned sometime it was okay to lower the standard a little bit for quick delivery to address urgent issue but be sure to make change to improve the standard later so to avoid accmulating technical debt.
